...
class Trainer(object):
    ...
    def train_epoch(self, epoch):
        ...
        train_data = Dataset(self.train_data, self.opt)
        ...
        for i, batch in enumerate(train_iter):
            ...
            pad_masks = sequence_mask(sequence_length=tgt_lens, max_len=tgt.size(0)).transpose(0,1)
            loss, num_corrects = self.model.backward(outputs, tgt, pad_masks, criterion=self.criterion)
            num_words = (tgt.data.ne(PAD).sum() + src.data.ne(PAD).sum()).item()
