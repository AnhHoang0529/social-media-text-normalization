...
import logging
#import lib
from loss import weighted_xent_loss, sequence_mask
from Dataset import Dataset
from utils import to_words, char_to_words, handle_tags, handle_unk, clean_self_toks
...
class Evaluator(object):
    def __init__(self, model, opt, unk_model=None):
        ...
        self.criterion = weighted_xent_loss

    def eval(self, data_iter, pred_file=None):
        self.model.eval()
        valid_data = Dataset(data_iter, self.opt)
        ...
        for i, batch in enumerate(val_iter):
            ...
            mask = sequence_mask(sequence_length=tgt_lens, max_len=tgt.size(0)).transpose(0,1)
            ...
            src = to_words(src, self.model.encoder.vocab)
            preds = to_words(predictions, self.model.decoder.vocab)
            tgt_sent_words = batch['tgt_sent_words']
            src_sent_words = batch['src_sent_words']

            if(self.opt.input=='char'):
                tgt_sent_words = char_to_words(tgt_sent_words)
                src_sent_words =  char_to_words(src_sent_words)
                preds = char_to_words(preds)
            ...
            preds = handle_tags(src_sent_words, preds)
            preds = handle_unk(src, src_sent_words, preds, self.unk_model, unk_file)
            if(self.opt.self_tok):
                preds = clean_self_toks(src_sent_words, preds, self.opt.self_tok)
                tgt_sent_words = clean_self_toks(src_sent_words, tgt_sent_words, self.opt.self_tok)

            sent_f1 = [f1([s], [p], [t], spelling=(self.opt.input=='spelling'))['f1'] for s, p, t in zip(src_sent_words, preds, tgt_sent_words)]
            all_inputs.extend(src_sent_words)
            all_preds.extend(preds)
            all_targets.extend(tgt_sent_words)
            all_others.extend([x for x in zip(indices, sent_f1)])
            total_loss += loss

        valid_loss =  total_loss/float(num_batches)
        results = f1(all_inputs, all_preds, all_targets, spelling=(self.opt.input=='spelling'))
    ...
    def _report(self, inputs, preds, targets, others):
        for input, pred, target, other in zip(inputs, preds, targets, others):
            ...
            logger.info('ind:{}\ninput:{}\ntarget:{}\nprediction:{}\n'.format(
                ind, token.join(input), token.join(target), token.join(pred)))

    def save_json(self, inputs, preds, targets, others, pred_file): #And csv!
        json_entries=[]
        for input, pred, target, other in zip(inputs, preds, targets, others):
            ind, sent_f1 = other
            json_entries.append({"index":ind,"output":pred,"input":input, "target":target, "score":sent_f1})
