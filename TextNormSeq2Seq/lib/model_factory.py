from model import EncoderRNN, LuongAttnDecoderRNN, Seq2Seq
...
def build_model(vocabs, opt):
    ...
    encoder = EncoderRNN(opt, src_vocab)
    decoder = LuongAttnDecoderRNN(opt, tgt_vocab)
    s2smodel = Seq2Seq(encoder, decoder, opt)
...
def create_optim(model, opt):
    ...
    return Optim(trained_params, opt.optim, opt.lr, opt.max_grad_norm,
