...
from constants import UNK_WORD, EOS_WORD, BOS_WORD, URL, HASH, MENTION
import functools
import torch
import logging
...
def clean_sentence(sent, remove_unk=False, remove_eos=True, remove_bos=True):
    if EOS_WORD in sent:
        sent = sent[:sent.index(EOS_WORD) + 1]
    if remove_unk:
        sent = filter(lambda x: x != UNK_WORD, sent)
    if remove_eos:
        if len(sent) > 0 and sent[-1] == EOS_WORD:
            sent = sent[:-1]
    if remove_bos:
        if len(sent) > 0 and sent[0] == BOS_WORD:
            sent = sent[1:]
    return sent
...
def handle_tags(input_words, pred_words):
    ...
    for input_tokens, pred_tokens in zip(input_words, pred_words):
        if URL in pred_tokens or HASH in pred_tokens or MENTION in pred_tokens:
            sent_length = min(len(input_tokens),len(pred_tokens))
            for i in range(sent_length):
                if(pred_tokens[i] == URL or pred_tokens[i] == HASH or pred_tokens[i] == MENTION):
...
def handle_unk(input, input_words, pred_words, unk_model,unkowns_file=None):
    if(unk_model):
        ...
        for input_tokens, input_words_tokens, pred_tokens in zip(input, input_words, pred_words):
            if UNK_WORD in input_tokens:
                sent_length = min(len(input_tokens),len(pred_tokens))
                for i in range(sent_length):
                    if(input_tokens[i]==UNK_WORD):
                        ...
                        trsl2wrds = to_words(translation, unk_model.encoder.vocab)
...
def copy_unks(input, input_words, pred_words):
    ...
    ret = []
    for input_tokens, input_words_tokens, pred_tokens in zip(input, input_words, pred_words):
        if UNK_WORD in input_tokens or UNK_WORD in pred_tokens:
            sent_length = min(len(input_tokens),len(pred_tokens))
            for i in range(sent_length):
                if(input_tokens[i] == UNK_WORD or pred_tokens[i] == UNK_WORD):
