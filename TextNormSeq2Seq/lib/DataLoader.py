...
from Tweet import Tweet, Preprocessor
from Dict import Dict
from constants import EOS, BOS, SELF
import random
#import lib
import json
import copy
...
class DataLoader(object):
    ...
    def encode_tweets(self):
        self.tweets_toIdx()
        src_sents, tgt_sents, tgt_sents_words, src_sents_words, indices = [], [], [], [], []
        for tweet in self.tweets:
...
def read_file(fn, valsplit=None):
    ...
    for tweet in data:
        ...
        ind = tweet['id']
        tweets.append(Tweet(src_tweet, tgt_tweet, ind))
    if(valsplit):
